...
/*   Updated: 2022/04/03 20:18:43 by areggie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//шаблонным, a typename T - обобщенный вид данных который будет использован в классе
// имя может быть рандомным
// слово typename можно заменить на слово class (это одно и тоже и непонятно почему два разных слова)
class MyClass
{
private:
	T1 value;
...
int main()
{
	...
	// MyClass c (a); 
	/* вот так объявить переменную не удастся, так как нужно указать 
	тип данных в угловых кавычкаx: <...>, 
	с которыми класс будет работать */
	
	
	MyClass<int, Point> c (a, p); 
	/* в угловых скобках можно указать 2 типа данных, 
	 так как у нас класс объявлен для работы с шаблоном из 2х видов переменных.
	 Расшифровка записи класса:
	 НазваниеКласса<типДанных1, типДанных2> НазавниеОбъекта(переменнаяТипаДанных1, переменнаяТипаД2) 
	 
	 Объект "c" класса MyClass принимает инт (2) и класс Point (p)
 	  */

	 
	c.DataTypeSize(); //для переменной класса Point размер 24 (потому что 3 double переменных)

	/* 	вывод размера инта = 4
		вывод размера double = 24 ( 3 x 8)
	*/
