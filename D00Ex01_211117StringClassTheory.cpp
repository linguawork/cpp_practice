/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   D00Ex01_211117StringClassTheory.cpp                :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: areggie <areggie@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/17 18:46:06 by areggie           #+#    #+#             */
/*   Updated: 2022/04/28 20:42:16 by areggie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
Day00 
ex00: 
std:: - пространство имен для STL

О классе std::string и о классе std::basic_string




std::string (в пространстве имен для всей STL обращение к пространству имен класса string ) 
https://www.cplusplus.com/reference/string/string/
класс string работает с char (1 байт)




Существует еще std::basic_string
Basic_string работает с разными типами char  (двубитный чар, wchar_t) UTF 

string String class (class )
wstring Wide string (class )
u16string String of 16-bit characters (class )
u32string String of 32-bit characters (class )



Разница между string and basic_string
https://stackoverflow.com/questions/1662107/whats-the-difference-between-stdstring-and-stdbasic-string-and-why-are-bot



https://www.cplusplus.com/reference/string/string/
ПО ссылке указаны методы класса STRING
MEMBER TYPES
MEMBER FUNCTIONS (METHODS)


DAY00 - EX00 используется функция TOUPPER
в CPP лучше использовать https://www.cplusplus.com/reference/locale/toupper/
https://en.cppreference.com/w/cpp/locale/locale //??
https://thispointer.com/converting-a-string-to-upper-lower-case-in-c-using-stl-boost-library/
*/

/* A few main functions from https://www.geeksforgeeks.org/stdstring-class-in-c/ */

//DAY00 - EX01

/*
запись строки через метод getline () https://www.cplusplus.com/reference/string/string/getline/?kw=getline
и запись в телефонную книгу


*/